...
async function loadGainsLog(days = DEFAULT_DAYS) {
  ...
  const { data, error } = await sb
    ...
    .gte("date", sinceStr)
    .order("date", { ascending: true });
  if (error) { console.error("gains_log error", error); sel("status").textContent = "Error cargando daily gains"; return []; }
  return data || [];
}
...
async function loadData() {
  ...
  sel("status").textContent = "Cargados " + snaps.length + " snapshots, " + gainsLog.length + " daily gains y " + deaths.length + " deaths (ultimos " + DEFAULT_DAYS + " dias)";
}
...
function computeKPIs(filteredSnaps, filteredGains, filteredDeaths) {
  ...
  if (lastDeathGlobal){
    ...
    sel("kpiLastDeathGlobal").textContent = "Ultimo: " + toDDMMYYYY_HHMM(lastDeathGlobal);
    let colorClasses;
    if (days <= 5) {
